 ...
 for(int i=0;;i++){
   ...
   scanf("%d",&user.Id);
     printf("\n\nplease type your estimated values as the following sequence : \n"
             ...
             "6.Enter the number of days of this month :\n");
             int a;
     for(a = 0;a<5;a++)
     {   printf("%d->",a+1);
    ...
    user.expenses[0]=user.expenses[0]*days;
    user.expenses[1]=user.expenses[1]*8*days;
    user.expenses[2]=user.expenses[2]*days;
system("cls");
   ...
   for(int i=0;;i++){
    ...
    if( mb<=user.expenses[0] || mb<=user.expenses[1] || mb<=user.expenses[2] || mb<=user.expenses[3] || mb<=user.expenses[4])
    {
       ...
       // printf("\nFood cost %.2f \nElectricity bill %.2f \nTransportation Cost %.2f ",user.suggest[0],user.suggest[1],user.suggest[2]);
        for(int r =0;r<3;r++)
        {
            final[r]= (user.expenses[r]-user.suggest[r])/days;
        }
        if(final[0]<0||(final[1]/8)<0||final[2]<0){
            printf("\n\nYou should earn more :( \n");
        }
        else{
